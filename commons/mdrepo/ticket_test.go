...
func TestTicket(t *testing.T) {
	t.Run("test GetMDRepoTicketStringFromToken", testGetMDRepoTicketStringFromToken)
	// t.Run("test AES", testAES)
	// t.Run("test SingleTicket", testSingleTicket)
	// t.Run("test MultiTickets", testMultiTickets)
}

func testGetMDRepoTicketStringFromToken(t *testing.T) {
	token := "COQgBZ_wfteci-nl-g-sUykApS8JaOe-mFWxow=="
	ticketString, err := GetMDRepoTicketStringFromToken("", token)
	if err != nil {
		t.Fatalf("Failed to get MDRepo ticket string from token: %v", err)
	}
	if ticketString == "" {
		t.Fatalf("Expected non-empty ticket string")
	}

	t.Logf("MDRepo ticket string from token: %s", ticketString)
}
